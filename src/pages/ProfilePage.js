...
// function GoogleIcon() {
//   return (
//     <svg width="24" height="24" className="google-icon" viewBox="0 0 24 24">
...
export default function ProfilePage() {
  ...
  const handleGoogleSignIn = async (response) => {
    try {
      ...
      // Send the credential (ID token) to your backend for verification and login
      ...
      const {user: usr } = data;
      if (token && usr) {
        console.log('Google sign-in successful:', data.token);
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      <div className="user-card">
        {user.profilePic
          ? <img className="user-avatar" src={user.profilePic} alt={user.name} />
          : <div className="user-avatar-fallback">{user.name?.charAt(0) || 'U'}</div>}
        <div>
          <div className="user-name">{user.name}</div>
          <div className="user-email">{user.email}</div>
          <div className="verified-badge">
            <CheckCircle size={12} color="var(--success)" /> Verified with Google
          </div>
        </div>
      </div>
